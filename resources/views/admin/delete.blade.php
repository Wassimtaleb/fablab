...
<head>
    ...
    <title>Supprimer un Admin - FabLab</title>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="bg-red-600 text-white p-4">
                <div class="flex items-center justify-between">
                    <h1 class="text-2xl font-bold">Supprimer un Administrateur</h1>
            ...
            <div class="p-6">
                <h2 class="text-lg font-semibold mb-4">Sélectionner un administrateur à supprimer</h2>
                
                @if($admins->isEmpty())
                    <p class="text-gray-500">Aucun administrateur disponible.</p>
                @else
                            ...
                            <tbody>
                                @foreach($admins as $admin)
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-2 px-4 border-b">{{ $admin->nom }}</td>
                                        <td class="py-2 px-4 border-b">{{ $admin->prenom }}</td>
                                        <td class="py-2 px-4 border-b">{{ $admin->email }}</td>
                                        <td class="py-2 px-4 border-b text-center">
                                            <button 
                                                class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 delete-admin"
                                                data-id="{{ $admin->id }}"
                                                data-nom="{{ $admin->nom }}"
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                Attention : Cette action est irréversible. Êtes-vous sûr de vouloir supprimer cet administrateur ?
                            </p>
                        </div>
                    </div>
                </div>
                <form id="delete-form" method="POST" action="{{ route('admin.destroy') }}" class="space-y-4">
                    @csrf
                    @method('DELETE')
                    ...
                    <div>
                        <p class="text-gray-700">Vous êtes sur le point de supprimer l'administrateur :</p>
                        <p class="font-semibold mt-2" id="admin-info"></p>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    adminInfoElement.textContent = `${prenom} ${nom} (${email})`;
