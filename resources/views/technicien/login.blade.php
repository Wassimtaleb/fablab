...
<div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
        <h2 class="text-2xl font-bold text-center mb-6">Connexion Technicien</h2>
        @if(session('error'))
        ...
        @endif
        <form method="POST" action="{{ route('technicien.login') }}" class="space-y-4">
            @csrf
            <div>
            ...
            <button type="submit" class="w-full rounded text-sm font-medium text-white bg-yellow-500 hover:bg-yellow-600 py-2">Se connecter</button>
        </form>
