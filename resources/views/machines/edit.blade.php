...
<head>
    ...
    <title>Modifier une Machine - FabLab</title>
...
<body class="bg-gray-100">
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-lg">
            <div>
                <h1 class="text-2xl font-bold text-center text-gray-900">Modifier une Machine</h1>
                @if ($errors->any())
            ...
            <form action="{{ route('admin.machines.update', $machine) }}" method="POST" enctype="multipart/form-data" class="mt-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" required
                              class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                              rows="4"
                              placeholder="Description détaillée de la machine">{{ old('description', $machine->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="statut" id="statut" required
                        ...
                        <option value="non_disponible" {{ old('statut', $machine->statut) == 'non_disponible' ? 'selected' : '' }}>Non disponible</option>
                ...
                <div>
                    ...
                    <img src="/images/machines/{{ $machine->image ?? strtolower(str_replace(' ', '-', $machine->nom)) . '.jpg' }}" 
                         alt="{{ $machine->nom }}" 
                         class="h-32 w-32 object-cover rounded-lg shadow-sm mb-2">
                </div>
